...
def test_v2_message_id_is_random_128_bit_hex() -> None:
    ...
    first = encrypt_message_v2(
        alice_device,
        bob_device.public_device(),
        b"first",
        created_at=1_800_000_000,
    )
    second = encrypt_message_v2(
        alice_device,
        bob_device.public_device(),
        b"second",
        created_at=1_800_000_000,
...
def test_v2_outer_metadata_tampering_is_detected(
    ...
    with pytest.raises(MessageV2DecryptionError, match=error):
        decrypt_message_v2(
            bob_device,
...
def test_v2_message_expiration_is_enforced() -> None:
    ...
    with pytest.raises(MessageV2DecryptionError, match="message has expired"):
        decrypt_message_v2(
            bob_device,
            alice_device.public_device(),
            message,

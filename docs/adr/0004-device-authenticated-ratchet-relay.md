...
Request signatures do not replace TLS. The current client refuses cleartext HTTP for non-loopback GhostNode URLs; HTTP remains available only for local loopback / SSH-tunnel operation. The protocol client also refuses every HTTP redirect rather than forwarding authenticated requests to a new target.

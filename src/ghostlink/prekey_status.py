"""Owner-authenticated GhostNode pre-key pool status protocol."""
...
_STATUS_VERSION = 1
_STATUS_DOMAIN = b"ghostlink-prekey-status-v1\x00"
...
class PreKeyStatusProofError(ValueError):
    """Raised when an owner status proof is malformed or unauthentic."""
...
def _canonical_status_bytes(
    *,
    ...
    if not _is_device_id(device_id):
        raise PreKeyStatusProofError("DeviceID is not canonical")
    ...
    if (
        ...
        raise PreKeyStatusProofError("issued_at is outside the supported range")
    ...
    return _STATUS_DOMAIN + json.dumps(
        document,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
...
class PreKeyStatusRequest(BaseModel):
    """Device-owner proof for reading one active relay pool status."""
    ...
    def validate_version(cls, value: int) -> int:
        if value != _STATUS_VERSION:
            raise ValueError("unsupported pre-key status request version")
        return value
    ...
    def validate_device_signing_public_key(cls, value: str) -> str:
        _decode_canonical_base64(
            value,
            field="device_signing_public_key",
...
class PreKeyStatusResponse(BaseModel):
    """Current public relay pool state returned only to the owning DeviceID."""
    ...
    def validate_version(cls, value: int) -> int:
        if value != _STATUS_VERSION:
            raise ValueError("unsupported pre-key status response version")
        return value
    ...
    def validate_device_id(cls, value: str) -> str:
        if not _is_device_id(value):
            raise ValueError("device_id must be a canonical DeviceID")
        return value
...
def create_prekey_status_request(
    ...
    *,
    ...
    canonical = _canonical_status_bytes(
        device_id=device.device_id,
        request_id=resolved_request_id,
        issued_at=issued_at,
    )
    signature = device.device.signing_key.sign(canonical).signature
    return PreKeyStatusRequest(
        version=_STATUS_VERSION,
        device_signing_public_key=base64.b64encode(signing_public_key).decode(
            "ascii"
        ),
        request_id=resolved_request_id,
        issued_at=issued_at,
...
def verify_prekey_status_request(
    request: PreKeyStatusRequest,
    route_device_id: str,
    *,
    now: int,
) -> None:
    """Verify fresh proof that the caller controls the route DeviceID."""
    if not _is_device_id(route_device_id):
        raise PreKeyStatusProofError("route DeviceID is not canonical")
    if request.issued_at > now + _CLOCK_SKEW_SECONDS:
        raise PreKeyStatusProofError("status request was issued too far in the future")
    if request.issued_at < now - _CLOCK_SKEW_SECONDS:
        raise PreKeyStatusProofError("status request is too old")
    ...
    if derive_device_id(signing_public_key) != route_device_id:
        raise PreKeyStatusProofError(
            "route DeviceID does not match device signing public key"
        )
    ...
    try:
        ...
        raise PreKeyStatusProofError("device status signature is invalid") from exc
